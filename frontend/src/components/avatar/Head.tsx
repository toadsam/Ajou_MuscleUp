...
export default function Head({ mbti, seedFeatures, strokeColor, skinColor, hairColor }: Props) {
  const baseEyeStyle = eyeByMbti(mbti) === "neutral" ? seedFeatures.eyeStyle : 0;
  const expression = expressionByMbti(mbti);
  const face = faceRadii[seedFeatures.faceShape % faceRadii.length];
  const faceRx = face.rx * seedFeatures.jawWidth;
  const faceRy = face.ry * seedFeatures.cheekDepth;
  ...
  return (
    <g>
      ...
      {seedFeatures.hairStyle === 0 && <path d={`M ${58 - seedFeatures.hairVolume * 3} 33 Q 72 10, ${86 + seedFeatures.hairVolume * 3} 33 L 88 37 Q 72 31 56 37 Z`} fill={hairColor} />}
      {seedFeatures.hairStyle === 1 && <path d={`M 56 36 Q 72 18, 88 36 Q 72 ${28 - seedFeatures.hairVolume * 2} 56 36 Z`} fill={hairColor} />}
      ...
      {seedFeatures.hairStyle === 4 && <path d={`M 55 37 Q 72 17, 89 37 Q 72 ${38 + seedFeatures.hairVolume * 2} 55 37`} fill={hairColor} />}
      ...
      {seedFeatures.hairStyle === 7 && <path d={`M 54 34 Q 72 20, 90 34 Q 72 ${24 - seedFeatures.hairVolume * 2} 54 34 Z`} fill={hairColor} />}
